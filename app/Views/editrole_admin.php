...
<body class="bg-gray-100 min-h-screen flex items-center justify-center">
    <div class="w-full max-w-xl bg-white p-8 rounded-lg shadow-md">
        <h1 class="text-2xl font-bold text-center text-gray-800 mb-6">Edit Role</h1>

        <form action="/auth/updaterole/<?= $role['id_role'] ?>" method="post" class="space-y-4">
            <div>
                <label for="id_role" class="block text-sm font-medium text-gray-700">ID Role</label>
                <input type="text" id="id_role" name="id_role" value="<?= $role['id_role'] ?>" required
            ...
            <div>
                ...
                <input type="text" id="role" name="role" value="<?= $role['role'] ?>" required
                       class="mt-1 block w-full px-4 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring focus:ring-blue-300">
            </div>

            <div class="flex justify-between items-center">
                <a href="/kelolarole_admin"
