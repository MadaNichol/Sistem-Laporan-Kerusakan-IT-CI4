...
<div class="container mx-auto px-4 py-4"> 
  <h1 class="text-4xl font-bold text-gray-800 mb-1">Vendor</h1>
  <p class="text-xl mb-4">Welcome, Admin</p>

  <div class="flex justify-between items-center mb-4">
    <!-- Tombol Tambah Vendor -->
    <a href="/auth/tambahvendor_admin" class="inline-block bg-blue-600 text-white px-4 py-2 rounded-md hover:bg-blue-700">
      Tambah Vendor
    </a>

    <!-- Kolom Pencarian -->
    <input type="text" id="searchInput" placeholder="Cari nama Vendor"
  ...
  <div class="w-full bg-white shadow rounded-lg overflow-x-auto">
    <table class="w-full table-auto">
      <thead class="bg-gray-100 border-b">
        <tr>
          <th class="px-4 py-2 text-center text-sm font-semibold text-gray-700">ID Vendor</th>
          <th class="px-4 py-2 text-center text-sm font-semibold text-gray-700">Nama Vendor</th>
          <th class="px-4 py-2 text-center text-sm font-semibold text-gray-700">Kontak Vendor</th>
          <th class="px-4 py-2 text-center text-sm font-semibold text-gray-700">Nama Perusahaan</th>
      ...
      <tbody>
        <?php foreach ($vendor as $vendor): ?>
          <tr class="border-b hover:bg-gray-50">
            <td class="px-4 py-2 text-center text-sm text-gray-600"><?= $vendor['id_vendor'] ?></td>
            <td class="px-4 py-2 text-center text-sm text-gray-600"><?= $vendor['nama_vendor'] ?></td>
            <td class="px-4 py-2 text-center text-sm text-gray-600"><?= $vendor['kontak_vendor'] ?></td>
            <td class="px-4 py-2 text-center text-sm text-gray-600"><?= $vendor['nama_perusahaan'] ?></td>
            <td class="px-4 py-2 text-center text-sm space-x-2">
              <!-- Edit -->
               <a href="/editvendor_admin/<?= $vendor['id_vendor'] ?>"
                class="text-yellow-500 hover:text-yellow-600" title="Edit">
                <i class="fas fa-edit text-lg"></i>
              </a>

              <!-- Delete -->
              <a href="/deletevendor/<?= $vendor['id_vendor'] ?>"
