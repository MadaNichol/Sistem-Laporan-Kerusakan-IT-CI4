...
<div class="container mx-auto px-4 py-4"> 
  <h1 class="text-4xl font-bold text-gray-800 mb-1">Staff IT</h1>
  <p class="text-xl mb-4">Welcome, Admin</p>

  <div class="flex justify-between items-center mb-4">
    <!-- Tombol Tambah Staff IT -->
    <a href="/auth/tambahstaff_it_admin" class="inline-block bg-blue-600 text-white px-4 py-2 rounded-md hover:bg-blue-700">
      Tambah Staff IT
    </a>

    <!-- Kolom Pencarian -->
    <input type="text" id="searchInput" placeholder="Cari Staff IT"
  ...
  <div class="w-full bg-white shadow rounded-lg overflow-x-auto">
    <table class="w-full table-auto">
      ...
      <tr>
        <th class="px-4 py-2 text-center text-sm font-semibold text-gray-700">ID Staff IT</th>
        <th class="px-4 py-2 text-center text-sm font-semibold text-gray-700">Nama Staff</th>
      ...
      <tbody>
      <?php foreach ($staffit as $staffit): ?>
        <tr class="bg-gray-50 border-b">
          <td class="px-4 py-2 text-center text-sm text-gray-600"><?= $staffit['id_staff_it'] ?></td>
          <td class="px-4 py-2 text-center text-sm text-gray-600"><?= $staffit['nama_staff_it'] ?></td>
          <td class="px-4 py-2 text-center text-sm text-gray-600"><?= $staffit['no_telp'] ?></td>
            <td class="px-4 py-2 text-center text-sm space-x-2">
              <!-- Edit -->
                  <a href="/editstaff_it_admin/<?= $staffit['id_staff_it'] ?>"
                  class="text-yellow-500 hover:text-yellow-600" title="Edit">
                  <i class="fas fa-edit text-lg"></i>
                </a>

          <!-- Delete -->
            <a href="/deletestaffit/<?= $staffit['id_staff_it'] ?>"
