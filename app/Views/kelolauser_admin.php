...
<div class="container mx-auto px-4 py-4"> 
  <h1 class="text-4xl font-bold text-gray-800 mb-1">User</h1>
  <p class="text-xl mb-4">Welcome, Admin</p>

  <div class="flex justify-between items-center mb-4">
    <!-- Tombol Tambah Role -->
    <a href="/auth/tambahuser_admin" class="inline-block bg-blue-600 text-white px-4 py-2 rounded-md hover:bg-blue-700">
      Tambah User
    </a>
  ...
  <div class="w-full bg-white shadow rounded-lg overflow-x-auto">
    <table class="w-full table-auto">
      ...
      <tr>
        <th class="px-4 py-2 text-center text-sm font-semibold text-gray-700">ID User</th>
        <th class="px-4 py-2 text-center text-sm font-semibold text-gray-700">Username</th>
        <th class="px-4 py-2 text-center text-sm font-semibold text-gray-700">Password</th>
      ...
      <?php foreach ($user as $user): ?>
        <tr class="bg-gray-50 border-b">
          <td class="px-4 py-2 text-center text-sm text-gray-600"><?= $user['id_user'] ?></td>
          <td class="px-4 py-2 text-center text-sm text-gray-600"><?= $user['username'] ?></td>
          <td class="px-4 py-2 text-center text-sm text-gray-600"><?= $user['password'] ?></td>
          <td class="px-4 py-2 text-center text-sm text-gray-600"><?= $user['id_role'] ?></td>
            <td class="px-4 py-2 text-center text-sm space-x-2">
              <!-- Edit -->
                  <a href="/edituser_admin/<?= $user['id_user'] ?>"
